        	...
        	dataset.open(dataset_filename);
          	...
          	if (dataset.is_open()) {
                ...
                for(auto it = m_pages_serialize.begin(); it != m_pages_serialize.end(); ++it) {
                	sdsl::load_from_file(tmp_bit_vector, "sdsl/" + std::to_string(it->second) + "a.sdsl");
                	sdsl::load_from_file(tmp_vector, "sdsl/" + std::to_string(it->second) + "b.sdsl");
                	sdsl::load_from_file(rmq, "sdsl/" + std::to_string(it->second) + "c.sdsl");
		...
		void serialize(const std::string& dataset_filename, const std::string& query_filename) {
			...
			if (dataset.is_open()) {
				...
				for(auto it = m_pages.begin(); it != m_pages.end(); ++it) {
					...
					sdsl::store_to_file(std::get<0>(it->second), "sdsl/" + std::to_string(id_sdsl) + "a.sdsl");
					sdsl::store_to_file(std::get<1>(it->second), "sdsl/" + std::to_string(id_sdsl) + "b.sdsl");
					sdsl::store_to_file(std::get<2>(it->second), "sdsl/" + std::to_string(id_sdsl) + "c.sdsl");

					std::ifstream in1("sdsl/" + std::to_string(id_sdsl) + "a.sdsl", std::ifstream::ate | std::ifstream::binary);
					std::ifstream in2("sdsl/" + std::to_string(id_sdsl) + "b.sdsl", std::ifstream::ate | std::ifstream::binary);
					std::ifstream in3("sdsl/" + std::to_string(id_sdsl) + "c.sdsl", std::ifstream::ate | std::ifstream::binary);
